...
outdir = os.path.abspath('tests/512-netcon')
...
cells = [Axax23(1,1,1), LTS23(2,2,2), Spinstel4(4,4,4), TuftIB5(5,5,5), TuftRS5(6,6,6), Bask56(7,7,7), Axax56(8,8,8), LTS56(9,9,9), NontuftRS6(10,10,10) ]
time = h.Vector().record(h._ref_t)
for i in range(7):
    cells[i].conect(cells[i+1])
...
stim1 = h.IClamp(cells[2].soma(0.5))
stim1.delay = 150
stim1.dur = 10
stim1.amp = 10
...
k = rxd.Species(ecs, name='k', d=2.62, charge=1, initial=lambda nd: 3 
...
def plot_cl(cell, time):
    fig = pyplot.figure(figsize=(20,16))
    ax1 = fig.add_subplot(2,1,1)
    pl = ax1.plot(time, cell.cl_vec)
    ax2 = fig.add_subplot(2,1,2)
    pl = ax1.plot(time, cell.cl_concentration)
    fig.savefig(os.path.join(k_na_dir, 'cl_%i.png' %cell.id))
    pyplot.close('all')
